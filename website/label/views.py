...
def index(request, url_category, uid, pid):
	...
	if uid == "1":
		...
		label = target_paper.label3
		unlabel_count = len(choosed_papers.filter(label3=""))
	...
	context = {"uid": uid, "pid": pid, "category": category, "users": users, "paper": target_paper, "prev_url": prev_url, "next_url": next_url, "bar": finish_percent, "url_category": url_category, "label": label, "index": pid+1, "sub_cates": cate_topics, "time": system_time, "preds":preds}
...
def update(request, url_category, uid, pid, label):
	...
	if str(uid)=="1":
		target_paper.label3 = label
		target_paper.time3 = time_diff
	elif uid=="2":
...
def list(request, url_category, uid):
	...
	count1 = 0
	count2 = 0
	choosed_papers1 = Paper.objects.filter(category=category, phased3__in=[1,3])
	choosed_papers2 = Paper.objects.filter(category=category, phased3__in=[2,3])
	total1 = len(choosed_papers1)
	total2 = len(choosed_papers2)
	choosed_papers1_labeled = choosed_papers1.filter(~Q(label3=""))
	choosed_papers2_labeled = choosed_papers2.filter(~Q(label4=""))
	count1 = len(choosed_papers1_labeled)
	count2 = len(choosed_papers2_labeled)
	print(count1/total1)
